...
class TrackerClass:
    ...
    def UpdateWithEvent(self, event):
        ...
        if self.State.Idle:
            return True
        if not self.State.Disengaged and ((self.Position[:2] - self.TM._OutOfBondsDistance < 0).any() or (self.Position[:2] + self.TM._OutOfBondsDistance >= np.array(self.TM.UsedGeometry)).any()): # out of bounds, cannot happen if disengaged
            if self.Lock:
