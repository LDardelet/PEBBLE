...
class SpeedTracker(Module):
    ...
    def __init__(self, Name, Framework, argsCreationReferences):
        ...
        self._TrackerLockedPosModDisplacementActivityPower = 0.2                    # Float. Exponent of the activity power of the activity used for tracker displacement correction. Default : 0.2
        ...
        if UpdateEvent:
            ...
            for Tracker in self.Trackers:
                ...
                if Tracker.Activity > 0:
                    self.TrackersAverageInternalPositions[-1] += [np.array(Tracker.MeanPositionSummed / Tracker.Activity)]
...
class TrackerClass:
    def __init__(self, TrackerManager, ID, InitialPosition):
        ...
        self.MeanPositionSummed = np.array([0., 0.])
    ...
    def UpdateWithEvent(self, event):
        ...
        if (self.Position < self.HalfSize).any() or (self.Position >= np.array(self.TM._LinkedMemory.STContext.shape[:2]) - self.HalfSize).any(): # out of bounds
            if self.Lock:
                self.Unlock('out of bounds death')
        ...
        if self.Activity < self.TM._DetectorDefaultWindowsLength * self.TM._TrackerMinDeathActivity: # Activity remaining too low
            if self.Lock:
                self.Unlock('low activity death')
            self.TM.KillTracker(self, event)
            return False
    ...
    def RunEvent(self, event):
        ...
        if (abs(event.location - self.Position)).max() > self.HalfSize:
            return False

        CurrentProjectedEvent = np.array([float(event.timestamp), event.location[0] - self.Position[0], event.location[1] - self.Position[1]])
        SavedProjectedEvent = np.array([event.timestamp, event.location[0] - self.Position[0], event.location[1] - self.Position[1]])
        ...
        self.SpeedError += SpeedErrorNorm
        ...
        if SpeedErrorNorm > 0:
            ...
            if np.linalg.norm(RotatedSpeedModOrientation) > 1.05:
                print(SpeedError, RotatedSpeedModOrientation)
            if SpeedMod[0] < 0:
        ...
        #if self.ScalarCorrectionActivity > self.TM._DetectorDefaultWindowsLength * self.TM._TrackerMinUpdateStatusRelativeActivity: # If enough points have allowed for a speed error computation
        #    if self.Status in [3,4]: # Any of Ap issue or Converged
        ...
        #            self.Status = 2 # We go back to stabilizing
        #    elif self.Status == 2:
        #        if abs(self.ScalarCorrectionValue) < self.ScalarCorrectionActivity * (self.TM._TrackerConvergenceThreshold - self.TM._TrackerConvergenceHysteresis):
        #            self.Status = 4 # We set as converged. Aperture issue is decided next
        #if self.ScalarCorrectionActivity > 0 and self.ApertureScalarEstimationByDelta / self.ScalarCorrectionActivity > self.TM._TrackerApertureIssueThreshold + self.TM._TrackerApertureIssueHysteresis: # If we detect an aperture issue
        #    if self.Status == 4:
        #        self.Status = 3
        #    elif self.Status == 5: # Here we suppose an aperture issue while we were locked. It either means that the initialization was false, or that we've drifter so much that only a line remains. Either way, we must unlock the feature
        #        self.Unlock("aperture issue")
        #elif self.ScalarCorrectionActivity > self.ApertureScalarEstimationByDelta * self.TM._TrackerApertureIssueThreshold - self.TM._TrackerApertureIssueHysteresis: # If no aperture issue remains
        #    if self.Activity <= self.TM._TrackerLockMaxRelativeActivity * self.TM._DetectorDefaultWindowsLength: # We want the activity to have stabilized
        #        if self.Status == 3:
        #            self.Status = 4
        #if self.Activity > self.TM._TrackerLockMaxRelativeActivity * self.TM._DetectorDefaultWindowsLength:
        #    if self.Status == 4:
        #        self.Status = 2
        #    elif self.Status == 5: # Here we suppose an aperture issue while we were locked. It either means that the initialization was false, or that we've drifter so much that only a line remains. Either way, we must unlock the feature
        #        self.Unlock("excessive activity")
        #if self.Status == 5 and self.ScalarCorrectionActivity < self.TM._TrackerLockedRelativeCorrectionsFailures * self.Activity:
        #    self.Unlock("unsufficient number of points matching")

        #if self.TM._TrackerAllowShapeLock and self.Status == 4:
        #    self.Status = 5
        #    self.LockedTime = CurrentProjectedEvent[0]
        #    self.LockedActivity = self.Activity
        #    self.LockedEvents = list(self.ProjectedEvents)
...
class Plotter:
    ...
    def GenerateTrackingGif(self, TrackerIDs = None, IgnoreTrackerIDs = [], AddVirtualPoint = False, SnapRatio = 1, tMin = 0., tMax = np.inf, Folder = '/home/dardelet/Pictures/GIFs/AutoGeneratedTracking/', BinDt = 0.005, add_ts = True, cmap = None, DoGif = True, addTrackersIDsFontSize = 0, DisplayedStatuses = ['Stabilizing', 'Converged'], DisplayedProperties = ['Aperture issue', 'Locked', 'None'], RemoveNullSpeedTrackers = 0, NoRemoval = False, GT = None, TrailDt = 0, TrailWidth = 2, ForcedColors = {}):
        ...
        print("Generating {0} png frames on {1} possible ones.".format(len(Snaps_IDs), len(self.TM.TrackersPositionsHistoryTs)))
