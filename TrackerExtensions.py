        ...
        for n_tracker, TrackerID in enumerate(TrackerIDs):
            ...
            if not TrackerColor is None:
                ...
                if addTrackersIDsFontSize:
                    ax.text(x, y + 3, str(TrackerID), color = TrackerColor, fontsize = addTrackersIDsFontSize, ha = 'center')

                if AddROI:
                    C = Circle((x,y), radius = S._TrackerDiameter / 2, color = TrackerColor, fill = False, linewidth = AddROI)
                    ax.add_artist(C)
...
#                        ax.plot(np.array(DataSerie)[:,0], np.array(DataSerie)[:,1], TrackerColor, ls = TrackerLineStyle, lw = TrailWidth)
    ...
    def GenerateTrackingGif(self, TrackerIDs = None, IgnoreTrackerIDs = [], AddVirtualPoint = False, SnapRatio = 1, tMin = 0., tMax = np.inf, Folder = '/home/dardelet/Pictures/GIFs/AutoGeneratedTracking/', BinDt = 0.005, add_ts = True, cmap = None, DoGif = True, addTrackersIDsFontSize = 0, DisplayedStatuses = ['Stabilizing', 'Converged', 'Locked'], DisplayedProperties = ['Aperture issue', 'OffCentered'], RemoveNullSpeedTrackers = 0, NoRemoval = False, GT = None, TrailDt = 0, TrailWidth = 2, ShowOrientation = False, MarkerSize = 4, AddROI = 2., FilePatern = 't_'):
        ...
        for nSnap, snap_id in enumerate(Snaps_IDs):
            ...
            self.CreateTrackingShot(TrackerIDs, IgnoreTrackerIDs, snap_id, BinDt, ax, cmap, addTrackersIDsFontSize, True, add_ts, DisplayedStatuses = DisplayedStatuses, DisplayedProperties = DisplayedProperties, RemoveNullSpeedTrackers = RemoveNullSpeedTrackers, VirtualPoint = VirtualPoint, GT = GT, TrailDt = TrailDt, TrailWidth = TrailWidth, ShowOrientation = ShowOrientation, MarkerSize = MarkerSize, AddROI = AddROI)
